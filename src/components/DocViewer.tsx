  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-y-auto theme-bg-primary theme-text-primary p-6 md:p-10 font-sans">
      ...
      <div className="flex items-center justify-between pb-6 mb-6 border-b theme-border">
        <div className="flex items-center gap-2 text-xs theme-text-muted font-mono">
          ...
          <button
            ...
            className="flex items-center gap-1 hover:text-amber-500 cursor-pointer transition-colors"
          >
            <Folder className="w-3.5 h-3.5 text-amber-500" />
            <span>{doc.category}</span>
          </button>
          <ChevronRight className="w-3.5 h-3.5 opacity-50" />
          <span className="theme-text-primary">{doc.slug}</span>
        </div>
          ...
          {isEditing ? (
            <>
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium theme-bg-card theme-bg-hover theme-text-secondary rounded-lg transition-all cursor-pointer border theme-border"
              >
              ...
              <button
                onClick={handleSave}
                disabled={isSaving}
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg transition-all cursor-pointer disabled:opacity-50 shadow-sm"
              >
            ...
            <>
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium theme-bg-card theme-bg-hover theme-text-primary rounded-lg border theme-border transition-all cursor-pointer shadow-xs"
              >
      ...
      {isEditing ? (
        <div className="space-y-4 mb-6">
          <div>
            ...
            <input
              ...
              className="w-full text-xl font-bold theme-bg-card border theme-border rounded-lg px-4 py-2 theme-text-primary focus:outline-none"
            />
          </div>
        ...
        <div className="mb-8">
          <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight theme-text-primary mb-4">
            {doc.title}
          </h1>
            ...
            {doc.tags && doc.tags.length > 0 && (
              <div className="flex items-center gap-1.5">
                ...
                <div className="flex gap-1">
                  {doc.tags.map((t) => (
                    <span key={t} className="px-2 py-0.5 rounded-full theme-bg-card theme-text-secondary text-[10px] font-mono border theme-border">
                      #{t}
                    </span>
                  ))}
                </div>
      ...
      {isEditing ? (
        ...
        <div className="prose max-w-none theme-text-primary leading-relaxed">
          <ReactMarkdown
            ...
            components={{
              ...
              code({ node, className, children, ...props }) {
                ...
                if (isInline) {
                  return (
                    <code className="px-1.5 py-0.5 rounded theme-bg-card theme-accent font-mono text-[13px] border theme-border" {...props}>
                      {children}
                    </code>
                  );
                }
