    ...
    <div className="flex-1 flex flex-col h-full overflow-y-auto theme-bg-primary theme-text-primary p-6 md:p-10 font-sans">
      ...
      <div className="pb-8 mb-8 border-b theme-border">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-3xl font-extrabold tracking-tight theme-text-primary">{project.name}</h1>
                <span className="px-2 py-0.5 rounded-md theme-bg-card theme-text-muted text-xs font-mono border theme-border">
                  {project.slug}
                </span>
              </div>
              <p className="text-xs theme-text-muted mt-1">{project.description || "Central Documentation & Logs Registry"}</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
          <div className="p-3 rounded-xl theme-bg-card border theme-border text-xs">
            <span className="theme-text-muted block text-[11px]">Database</span>
            <span className="theme-accent font-semibold flex items-center gap-1 mt-0.5">
              <Database className="w-3.5 h-3.5" /> Cloudflare D1
            </span>
          </div>
          <div className="p-3 rounded-xl theme-bg-card border theme-border text-xs">
            <span className="theme-text-muted block text-[11px]">Documents</span>
            <span className="theme-text-primary font-semibold text-sm mt-0.5">{docs.length} Pages</span>
          </div>
          <div className="p-3 rounded-xl theme-bg-card border theme-border text-xs">
            <span className="theme-text-muted block text-[11px]">Categories</span>
            <span className="text-amber-500 font-semibold text-sm mt-0.5">{categories.length} Folders</span>
          </div>
          <div className="p-3 rounded-xl theme-bg-card border theme-border text-xs">
            <span className="theme-text-muted block text-[11px]">Features</span>
            <span className="text-cyan-500 font-semibold text-sm mt-0.5">{features.length} Epics</span>
          </div>
        </div>
      </div>
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-[11px] font-mono px-2 py-0.5 rounded-full theme-bg-secondary theme-text-muted border theme-border">
                ...
                <div className="flex items-center justify-between pt-4 mt-3 border-t theme-border text-xs theme-text-muted group-hover:text-amber-500">
      ...
      <div className="rounded-2xl border theme-border theme-bg-card p-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            className="text-xs text-purple-500 hover:opacity-80 flex items-center gap-1 cursor-pointer font-medium"
          >
